    ...
    public void initialize() {
        try {
            ...
            if (account == null) {
                ...
                updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.HANDLER_INITIALIZING_ERROR, "No Easee account while initiatizing ChargerThing, null");
                return;
            }
            api = account.getAPI();
...
/**
 * {@link updateChargerFromCloud} updates charger Thing channels and configuration properties based on cloud state information
 *  Changes the charger Thing to OFFLINE if there was a problem getting state information or if the charger is disabled (through native app or otherwise)
 * 
 */
    public void updateChargerFromCloud() {
        try {
            ...
            if (chargerState == null) {
                logger.debug(
                        "Failed to update state from API, Charger {} changed to OFFLINE. Will try again later", 
                        id);
            ...
            updateState(NEW_FIRMWARE_AVAIL, (chargerState.latestFirmware.equals(chargerState.chargerFirmware)) ? OnOffType.OFF : OnOffType.ON);
